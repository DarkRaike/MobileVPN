...
> **Молчаливый обрыв №2.** Если у провайдера есть собственный сетевой фильтр
> перед машиной, он важнее хостового: заготовка обычно открывает `22`, `80` и
> `443`, а `8443` остаётся закрытым. Снаружи это выглядит как полностью
> исправный сервис при нулевом трафике VPN. Такое правило добавляется в панели
> провайдера отдельно от `ufw`.
...
Различать отказы важно: закрытый порт роняет само TCP-соединение
(`connection refused` или таймаут на SYN). Если TCP открывается, а TLS следом
не завершается — порт открыт, и режут уже установленное соединение по
содержимому. Это не firewall, см. «Порт VLESS» ниже.
...
Всё выше может быть верно, а клиент всё равно не подключается — если соединение
режут в пути. Отличается это от любой поломки сервера одним измерением с
клиентской сети:

```bash
node -e "const t=require('node:tls'),n=require('node:net');const a=Date.now();const s=n.connect({host:'vpn.example.com',port:8443,timeout:8000});s.on('connect',()=>{console.log('TCP OPEN',Date.now()-a,'ms');s.destroy();const b=Date.now();const x=t.connect({host:'vpn.example.com',port:8443,servername:'www.nvidia.com',rejectUnauthorized:false,timeout:8000},()=>{console.log('TLS OK',Date.now()-b,'ms');x.destroy()});x.on('timeout',()=>{console.log('TLS TIMEOUT');x.destroy()});x.on('error',e=>console.log('TLS',e.code))});s.on('timeout',()=>{console.log('TCP TIMEOUT');s.destroy()});s.on('error',e=>console.log('TCP',e.code))"
```

`TCP OPEN` вместе с `TLS TIMEOUT` означает, что порт открыт и сервер жив, а
гасится уже установленное соединение — по содержимому, после ClientHello. Для
сравнения TLS на `443` того же адреса при этом проходит.

Проверить, что сервер здесь ни при чём, можно эталонным клиентом с самого VPS:
он ходит на тот же публичный адрес и порт, но не пересекает фильтр. Если оттуда
трафик идёт, а с клиентской сети нет — дело в пути.

`8443` давно в списке целевых портов у фильтрующего оборудования. Варианты, в
порядке устойчивости:

1. **REALITY на `443` отдельного IP.** Второй адрес на машине: Caddy остаётся на
   `<ip1>:443`, Xray встаёт на `<ip2>:443` через привязанную публикацию
   `"<ip2>:443:443"`, `VLESS_PORT=443`, `REALITY_ENDPOINT_HOST=<ip2>`. `443`
   нельзя заблокировать по номеру, не сломав весь HTTPS.
2. **Другой порт.** `VLESS_PORT` — переменная стека, кода менять не нужно.
   Порты вида `2053`, `2083`, `2087`, `2096` разбираются как обычный HTTPS.
   Решение временное: заблокируют и их.

`tech.md` §10.6 фиксирует этот выбор как отдельное измеренное решение.
...
1. **Соединение режут в пути на `8443`.** TCP открывается, TLS не завершается,
   на `443` того же адреса всё работает. Сервер при этом исправен полностью —
   см. «Порт VLESS».
...
3. **`8443/tcp` закрыт фильтром провайдера.** Хостовый `ufw` открыт, а панель
