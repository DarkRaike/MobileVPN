...
> **Молчаливый обрыв №1.** Имя домена — часть системы, а не косметика. Клиент
> отправляет его открытым текстом в SNI при каждом подключении, и фильтрующее
> оборудование принимает решение по этому имени, ещё не видя трафика. Домен
> вида `vpn-service.fun` попадает в списки автоматически: слово в имени и
> «дешёвая» зона. После этого не работает ничего — ни туннель, ни Mini App, ни
> выдача подписок, при полностью исправном сервере.
>
> Берите имя, по которому назначение не читается: без `vpn`, `proxy`, `tunnel`,
> `secure`, в зоне `.com`, `.net`, `.org` или `.ru`. Оно должно выглядеть как
> сайт студии, блог или личный проект.

Две `A`-записи на публичный IP VPS:

| Тип | Имя   | Роль                                   |
| --- | ----- | -------------------------------------- |
| A   | `app` | Mini App, webhook и сам туннель на 443 |
| A   | `sub` | Marzban `/sub/*`                       |

Отдельной записи для VPN больше нет: туннель живёт секретным путём на `app`.

Если DNS обслуживает Cloudflare, проксирование должно быть выключено (серое
облако): оранжевое подменяет TLS и ломает туннель.

```bash
dig +short app.example.com sub.example.com
```

Проверить, что имя проходит через фильтр целевой сети, можно только оттуда —
откройте `https://app.example.com/healthz` с устройства в этой сети **без VPN**.
Пустой ответ или таймаут означают, что домен непригоден, и дальше настраивать
нечего.
...
## 11. Как устроен туннель

Caddy владеет публичным `443` и терминирует TLS своим сертификатом. Один
неугадываемый путь на хосте `app` проксируется в Xray по WebSocket; всё
остальное на этом домене — обычный Mini App.

Снаружи виден один HTTPS-запрос к обычному сайту с настоящим сертификатом,
потому что это он и есть. Нестандартного рукопожатия, которое можно опознать по
отпечатку, в схеме нет — этим она отличается от REALITY, который на фильтрующих
сетях опознаётся и гасится.

Путь генерируется один раз, хранится вместе с остальными секретами и в
репозиторий не попадает. Порт Xray наружу не публикуется вообще.

Проверить транспорт можно снаружи, не поднимая VPN-клиент:

```bash
node -e "const s=new WebSocket('wss://app.example.com<ПУТЬ>');s.addEventListener('open',()=>{console.log('101 UPGRADE OK');s.close()});s.addEventListener('error',e=>console.log('ERROR',e.message||''))"
```

`101 UPGRADE OK` означает, что путь доходит до Xray. Ошибка на случайном пути —
нормально: туда отвечает Mini App.
...
1. **Имя домена опознаётся фильтром.** Оно уходит открытым текстом в SNI, и
   решение принимается по нему: не работает ни туннель, ни Mini App, ни выдача
   подписок, а сервер при этом исправен. Проверяется открытием
   `https://app.<domain>/healthz` из целевой сети без VPN.
...
мониторинга, и первый из них не лечится ничем на сервере. Начинайте разбор с `scripts/vpn-diagnose.mjs`.
