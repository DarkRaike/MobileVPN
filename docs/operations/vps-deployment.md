...
```bash
openssl s_client -connect www.swift.com:443 -servername www.swift.com -tls1_3 -groups X25519MLKEM768:X25519 </dev/null 2>/dev/null | grep -E "Negotiated TLS1.3 group|New, TLSv1.3"
```

Строка `Negotiated TLS1.3 group: X25519MLKEM768` означает, что цель непригодна.
Отсутствие этой строки при `New, TLSv1.3` означает классический X25519 и
пригодную цель.

Проверено на 2026-08-01: `www.swift.com`, `yahoo.co.jp`, `www.nvidia.com` и
`www.samsung.com` остаются на классическом X25519; `www.microsoft.com` и
`dl.google.com` выбирают гибрид и для этой сборки не подходят.
